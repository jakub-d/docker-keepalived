...
generate_virtualips() {
	# generate VIRTUAL_IPS entries
	for ip in $(echo "${VIRTUAL_IP}")
	do
		echo -n "        ${ip}/32 dev ${INTERFACE}\\n"
	done
}
...
if [ -z "${INTERFACE}" ]; then
	# Choose the public interface on the host
	INTERFACE=$(ip route get 8.8.8.8 | awk '{ print $(NF-2); exit }')
else
	# The INTERFACE variable can be a list of potential interfaces (space separated)
	INTERFACE=$(select_existing_interface "${INTERFACE}")
fi

PRIORITY=$(( ( RANDOM % 200 ) +1 ))

# router_id equals the last octet of the ip address
VRID="${INSTANCE_ID}"
PASS="${PASS:-default}"

VIRTUAL_IPS=$(generate_virtualips)

# Substitute variables in config file.
/bin/sed -i "s|{{VIRTUAL_IPS}}|${VIRTUAL_IPS}|" /etc/keepalived/keepalived.conf
/bin/sed -i "s/{{VRID}}/${VRID}/" /etc/keepalived/keepalived.conf
/bin/sed -i "s/{{PASS}}/${PASS}/" /etc/keepalived/keepalived.conf
...
for ip in $(echo "${VIRTUAL_IP}")
do
  if [[ $(ip addr | grep $INTERFACE | grep $ip) ]]
    then
